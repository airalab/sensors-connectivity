...
def main():
    ...
    with open(ino + "/src/mobile_gps.ino", "r") as f:
        firmware = f.read()
    ...
    firmware = firmware.replace("$$STASSID$$", "\"{}\"".format(settings["stassid"])) \
                ...
                .replace("$$RXPIN$$", str(settings["rxpinsensor"])) \
                .replace("$$TXPIN$$", str(settings["txpinsensor"])) \
                .replace("$$RXPINGPS$$", str(settings["rxpingps"])) \
                .replace("$$TXPINGPS$$", str(settings["txpingps"])) \
